...
export default function CreateEdit() {
  ...
  const handleDemandPVMSSubmit = (e) => {
    ...
    setIsFormSubmited(true)
      const data = new FormData();
      // const uurl =  '/pvms-stock-del' ;
      data.append('data', JSON.stringify(request_data))

 // alert("Submitting : " + window.app_url +'/pvms-stock-del' + "\n\nPayload:\n" + JSON.stringify(request_data, null, 2));
      axios.post(window.app_url + '/pvms-stock-del', data).then((res) => {

          console.log(res.data,'crate Issue');
          setIsFormSubmited(false)
          if(res.data.status){

              window.location.href = '/pvms-stock-del'     // window.pvmsStockAddUrl
          }
      }).catch((err) => {
           // alert(res.data) ;
          setIsFormSubmited(false)
            ...
            <div className='d-flex justify-content-end gap-2'>
              ...
              {(demadType != 4 && demandPVMS.length == 0) || (demadType == 4 && repairPVMS.length == 0) ?
                ...
                <div className='text-right'>
                  ...
                  <button className='btn btn-success' disabled={isFormSubmited || DemandNoChecking}>{isFormSubmited ? 'Saving...' : 'Save OUT'}</button>
                </div>
              }
            </div>
          </div>
...
if (document.getElementById('react-receivePvmsBatchStockDel')) {
  createRoot(document.getElementById('react-receivePvmsBatchStockDel')).render(<CreateEdit />)
}
