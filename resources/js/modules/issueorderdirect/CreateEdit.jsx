...
export default function CreateEdit() {
  ...
  const handleDemandPVMSSubmit = (e) => {
    ...
    if (demandId) {
      axios.put(window.app_url + '/issue-order-direct-store/' + demandId, request_data).then((res) => {

        // window.location.href = window.issue_order_direct_index_url
      ...
      const data = new FormData();
      data.append('data', JSON.stringify(request_data))
      axios.post(window.app_url + '/issue-order-direct-store', data).then((res) => {
    ...
    <>
      ...
      <form onSubmit={handleDemandPVMSSubmit}>
        ...
        <div className='col-6 mb-2'>
            <b>Organization:<span className='text-danger'>*</span> </b>
            <select onChange={(e) => handleChangeOrganization(e)} value={organigationId} className='form-control' required={true} >
                <option value="">Select</option>
                {organigations.map((val, key) => (
                    <option key={key} value={val.id}>{val.name}</option>
                ))}
            </select>
        </div>
              ...
              <div className='col-md-9'>
                ...
                <div className='row'>
                  ...
                  <div className='col-6 mb-2'>
                    {!demandId && <>
                      {demandDate && <>
                        <b>Voucher Number:<span className='text-danger'>*</span>  <label><input type='checkbox' onChange={(e) => { setIsCustomDemandNo(e.target.checked); setDemandNo('') }} checked={isCustomDemandNo} /> Custom number</label> </b>
                ...
                <div className='col-6 mb-2'>
                    <b>Send To:<span className='text-danger'>*</span> </b>
                    <select onChange={(e) => setSendToOrgId(e.target.value)} value={sendToOrgId} className='form-control' required={true}>
                        <option value="">Select</option>
                        {sendToOrgs.map((val, key) => (
                            <option key={key} value={val.id}>{val.name}</option>
                        ))}
                    </select>
                </div>
              ...
              <table className='table'>
                <thead>
                  <tr>
                    ...
                    <th className='text-right width-150'>Issue Qty. <span className='text-danger'>*</span></th>
                    <th></th>
                  </tr>
                </thead>
                <tbody>
            ...
            <div className='d-flex justify-content-end gap-2'>
              ...
              {(demadType != 4 && demandPVMS.length == 0) || (demadType == 4 && repairPVMS.length == 0) ?
                ...
                <div className='text-right'>
                  ...
                  <button className='btn btn-success' disabled={isFormSubmited || DemandNoChecking}>{isFormSubmited ? 'Saving...' : 'Issue Save'}</button>
                </div>
              }
            </div>
          </div>
...
if (document.getElementById('react-issueorderdirect')) {
  createRoot(document.getElementById('react-issueorderdirect')).render(<CreateEdit />)
}
