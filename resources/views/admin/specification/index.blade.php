    ...
    <div class="app-main__outer">
        <div class="app-main__inner">
            <div class="col-lg-12 app-content">
                <div class="app-content-top-title">Spcifications</div>
...
{{--                        <h5 class="card-title">Table striped</h5>--}}
                        <div class="d-flex justify-content-between align-items-center table-header-bg py-1 mb-1">
                            <h5 class="f-14">Specifications</h5>
{{--                            {{ $specifications->links('vendor.pagination.custom') }}--}}
                            @can('Create Specification')
                            <a class="nav-link" href="{{route('add.specification')}}">
                                    ...
                                    <table id="example" class="table-width-100 table table-hover table-striped table-bordered">
                                        ...
                                        @foreach($specifications as $k=>$specification)
                                            <tr>
                                                ...
                                                <td>{{$specification->uname}}</td>

                                                <td class="d-flex">
                                                    @can('Edit Specification')
                                                        <a href="{{url('/settings/edit/specification/'.$specification->id)}}">
                                                    ...
                                                    @can('Delete Specification')
                                                        <form action="{{url('/settings/delete/specification/'.$specification->id)}}" method="post">
                                                            @csrf
                                                            <button type="submit" id="{{$specification->id}}" class="border-0 btn-transition btn btn-outline-danger delete-account-unit"><i class="fa fa-trash-alt"></i></button>
                                                        </form>
                                                    @endcan
                                                </td>
                                            </tr>
