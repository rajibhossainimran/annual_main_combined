                ...
                <div class="main-card app-content-inner card">
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-center table-header-bg py-1 mb-1">
                            <h5 class="f-14">Supplier</h5>
{{--                            {{ $services->links('vendor.pagination.custom') }}--}}
                            @can('Create Service')
                            <a class="nav-link" href="{{route('add.supplier')}}">
                                    ...
                                    <table id="example" class="table-width-100 table table-hover table-striped table-bordered">
                                        ...
                                        @foreach($suppliers as $k=>$supplier)
                                            <tr>
                                                ...
                                                <td class="d-flex">
                                                    @can('Edit Service')
                                                        <a href="{{url('/settings/edit/supplier/'.$supplier->id)}}">
                                                    ...
                                                    @endcan
                                                    @can('Delete Service')
                                                        <form action="{{url('/settings/delete/supplier/'.$supplier->id)}}" method="post">
                                                            @csrf
                                                            <button type="submit" id="{{$supplier->id}}" class="border-0 btn-transition btn btn-outline-danger delete-account-unit"><i class="fa fa-trash-alt"></i></button>
                                                        </form>
                                                    @endcan
                                                </td>
                                            </tr>
