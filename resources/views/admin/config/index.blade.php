                ...
                <div class="main-card card app-content-inner">
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-center table-header-bg py-1 mb-1">
                            ...
                            @can('Create Config')
                                <a class="nav-link" href="{{route('add.config')}}">
                                    <button class="btn-icon btnc btn-custom">
                                        <i class="fa fa-plus btn-icon-wrapper"></i>
                                        Create Password Reset Date
                                    </button>
                                </a>
                            @endcan
                        </div>
                                    ...
                                    <table id="example" class="table-width-100 table table-hover table-striped table-bordered">
                                        ...
                                        @foreach($settings as $k=>$data)
                                            <tr>
                                                ...
                                                <td class="d-flex">
                                                    @can('Edit Config')
                                                        <a href="{{url('/settings/edit/config/'.$data->id)}}">
                                                    ...
                                                    @endcan
                                                    @can('Delete Config')
                                                        <form action="{{url('/settings/delete/config/'.$data->id)}}" method="post">
                                                            @csrf
